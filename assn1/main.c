...
void doubleCapacity(List* myList) {
	myList->capacity *= 2;
	Entry** newData = malloc(sizeof(Entry*)*myList->capacity);
	for(int i = 0; i < myList->size; i++) {
...
void halveCapacity(List* myList) {
	myList->capacity /= 2;
	for(int i = myList->size-1; i >= myList->capacity; i--) {
		freeEntry(myList->data[i]);
	}
...
void shiftList(List* myList, int pos, bool right) {
	if(right) {
		// make space if necessary
		if(myList->capacity == myList->size) {
			doubleCapacity(myList);
		}
		for(int i = myList->size; i > pos; i--) {
			myList->data[i] = myList->data[i-1];
		}
		myList->data[pos] = NULL;
		myList->size++;
	} else {
		freeEntry(myList->data[pos]);
		for(int i = pos; i < myList->size-1; i++) {
			myList->data[i] = myList->data[i+1];
		}
		myList->data[myList->size-1] = NULL;
		// update size, then check to see if we need to halve capacity
		myList->size--;
		if(myList->capacity > myList->size*2) {
...
void insertToHead(List* myList, char* name, char* lastname, float height, int age) {
	// if list is not empty then shift to make space
	if(myList->size) {
		shiftList(myList, 0, true);
	}else {
		myList->size++;
	}
...
void insertToPosition(List* myList, int position, char* name, char* lastname, float height, int age) {
	...
	shiftList(myList, position, true);
...
void deleteFromHead(List* myList) {
	shiftList(myList, 0, false);
}

void deleteFromTail(List* myList) {
	shiftList(myList, myList->size-1, false);
}

void deleteFromPosition(List* myList, int position) {
	shiftList(myList, position, false);
}
